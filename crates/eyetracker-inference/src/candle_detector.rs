...
impl CandleDetector {
    ...
    pub fn from_pretrained(_path: impl AsRef<Path>) -> Result<Self, DetectionError> {
        ...
        {
            Err(DetectionError::ModelLoad(
                "candle feature not enabled; rebuild with --features candle"
                    .to_string(),
            ))
        }
    }
...
impl FaceDetector for CandleDetector {
    fn detect(&self, frame: &[u8], width: u32, height: u32) -> Result<Vec<FaceBox>, DetectionError> {
        if frame.is_empty() {
...
mod tests {
    ...
    fn synthetic_mlp_outputs_bounded() {
        ...
        for seed in 0..16_u32 {
            ...
            assert!(p >= -1.0 && p <= 1.0, "pitch {} out of bounds for seed {}", p, seed);
            assert!(y >= -1.0 && y <= 1.0, "yaw {} out of bounds for seed {}", y, seed);
        }
    }
